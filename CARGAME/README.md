...
Processing is implemented in `car_env_ai.py` and `cnn_agent.py`.
...
*(Fixed numbering — original list skipped some numbers.)*

### **Reward Function**

Defined in `car_env_ai.py`:

* **+0.1** reward per step survived
* **+1** reward when an obstacle (car) is successfully passed
...
The agent uses **Deep Q-Learning (DQN)** with:

* CNN feature extractor
* Experience Replay
* ε-greedy exploration
* Target network updates (if implemented in your code)
